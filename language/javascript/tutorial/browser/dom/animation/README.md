# 动画

- https://drafts.csswg.org/web-animations/
- [Web Animations API](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Animations_API)
- [Using the Web Animations API](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Animations_API/Using_the_Web_Animations_API)
- [Web animations examples](https://mozdevs.github.io/Animation-examples/)
- [web-animations-js](https://github.com/web-animations/web-animations-js)
- [Are we animated yet?](https://birtles.github.io/areweanimatedyet/)
- [WAAPI Browser Support Test](https://codepen.io/danwilson/pen/xGBKVq)
- [New web animations engine in blink drives CSS animations and transitions](https://developer.chrome.com/blog/new-web-animations-engine-in-blink-drives-css-animations-transitions/)
- [Introducing Web Animations](https://birtles.wordpress.com/2013/06/26/introducing-web-animations/)
- [web-animations-demos](https://github.com/web-animations/web-animations-demos)
- [Web Animations - element.animate() is now in Chrome 36](https://developers.google.com/web/updates/2014/05/Web-Animations-element-animate-is-now-in-Chrome-36)

---

- [Precise Timing With Web Animations API](https://www.smashingmagazine.com/2022/06/precise-timing-web-animations-api/)
