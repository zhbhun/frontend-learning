# ECMAScript

- https://tc39.es
- [Ecma International](https://www.ecma-international.org/)
- [Ecma TC39](https://github.com/tc39)
- [JavaScript language resources](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Language_Resources)

## 检测

- [前端早读课【第1295期】浅谈 instanceof 和 typeof 的实现原理](hhttps://mp.weixin.qq.com/s?__biz=MjM5MTA1MjAxMQ==&mid=2651228868&idx=1&sn=1bae0dda389c612463b705e9e9cef4ef)
- [前端早读课【第1745期】现代 JavaScript 教程 - 类型检测："instanceof"](https://mp.weixin.qq.com/s/nmj9dQWRyaEURpearFHULA)

## 问题

### JavaScript 的不严谨之处

- parseInt 在没有指定基数时不同环境执行结果不一致，且 parseInt 支持待转换字符中包含非数字字符；

## 参考文献

- [读懂 ECMAScript 规格](http://www.ruanyifeng.com/blog/2015/11/ecmascript-specification.html)
- [Practical Modern JavaScript](https://ponyfoo.com/books/practical-modern-javascript/chapters)
