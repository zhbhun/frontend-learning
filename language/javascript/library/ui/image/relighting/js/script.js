...
function startProcessing() {
  ...
  document
    .getElementById("loader-overlay-div")
    .setAttribute("style", "display: none; visibility: hidden;");
  lightPosSpan.innerHTML =
    "[" +
    lightPos[0].toFixed(2) +
    ", " +
    lightPos[1].toFixed(2) +
    ", " +
...
function sliderUpdate() {
  lightPos = [
    xlightSlider.value / 100,
    ylightSlider.value / 100,
...
function setCanvasSize() {
  ...
  var ratio = 1;
  if (
    window.innerWidth * ratio < imgSize[0] ||
    window.innerHeight * ratio < imgSize[1]
  ) {
...
function setupShaderAttributes() {
  ...
  imgBuffer.positionBuffer.numItems =
    mesh.length / imgBuffer.positionBuffer.itemSize;
  ...
  imgBuffer.normalBuffer.numItems =
    normals.length / imgBuffer.normalBuffer.itemSize;
  ...
  gl.texImage2D(
    ...
    gl.RGBA,
    gl.UNSIGNED_BYTE,
    document.getElementById("src-tex-img")
  );
  ...
  shaderProgram.textureLighting = gl.getUniformLocation(
    shaderProgram,
    "textureLighting"
  );
  shaderProgram.lightIntensity = gl.getUniformLocation(
    shaderProgram,
    "lightIntensity"
  );
...
function draw() {
  ...
  gl.vertexAttribPointer(
    shaderProgram.positionAttr,
    imgBuffer.positionBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, imgBuffer.normalBuffer);
  gl.vertexAttribPointer(
    shaderProgram.normalAttr,
    imgBuffer.normalBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.uniform2fv(
    shaderProgram.imgSizeUnif,
    new Float32Array(ImgHelper.getImageSize())
  );
  gl.uniform2fv(
    shaderProgram.minMaxZUnif,
    new Float32Array([ImgHelper.minZ, ImgHelper.maxZ])
  );
...
function setupImageSelector() {
  ...
  reloadButton.addEventListener("click", function () {
    ...
    window.location.href =
      window.location.href.split("?")[0] + "?" + selectedOption.value;
  });
}
