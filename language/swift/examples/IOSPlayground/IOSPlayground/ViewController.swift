...
class ViewController: UICollectionViewController {
	...
	override func viewDidLoad() {
		...
		collectionView.register(SectionItem.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "header")
	...
	override func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
		return section == 0 ? viewControllers.count : styleControllers.count
	}
	
	override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? ListItem
		let controllers = indexPath.section == 0 ? viewControllers : styleControllers
	...
	override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
		let header = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "header", for: indexPath) as? SectionItem
		var text = ""
...
fileprivate class ListItem: UICollectionViewCell {
	...
	func configure(with text: String, isFirst: Bool, isLast: Bool) {
		...
		if (isLast) {contentView.layer.cornerRadius = 10
