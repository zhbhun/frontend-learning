...
- 全局状态
    ...
    - 系统设备：系统类型、系统语言、设备分辨率
    - 业务状态：是否订阅、是否登录

- 页面状态

    - 路由状态：一些路由传参，实际会转变为业务状态
    - 业务状态：是否草稿、是否关注、是否收藏和是否点赞等

- 组件状态

    - 交互状态：悬浮、按压、聚焦
    - 业务状态：选中、激活、禁用、类型等
    - 衍生状态：tab 项选中状态(根据 tabs 的选中选计算得出)

- 组合状态：分辨率 > 语言 > 登录 > 订阅 > 交互.禁用

变体机制

- 样式属性：

    - 全局状态：在根节点上根据状态来切换类名，然后元素上的样式带上根节点的状态样式类进行覆盖样式
    - 交互状态：获取到对应的 DOM 节点，然后监听交互事件，然后给 DOM 节点添加对应的状态样式类

        ps：这种方式是否可能容易出现 bug，例如：React DOM 节点发生变化时，状态样式类可能丢失。

    - 组件状态：组件内部需要自行提供对应的状态样式类供外部定制

- 非样式属性：

    每个组件的非样式属性需要使用状态来管理，根据不同的状态叠加生成最终的属性值。

    ps：属性、交互和动画是否要支持变体？

- 变体优先级顺序：媒体查询 > 语言|系统|登录|订阅 > 交互|变体

状态管理

- 组件自己维护：通用的交互状态需要自己实现，存在一些重复，组件联动不好实现
- 组件包装层维护：

    - 交互状态样式通过 CSS 伪类选择器来实现
    - 业务状态通过组件声明的变体类来实现
    - 组件非样式属性需要按变体优先级顺序层叠计算
    - 组件插槽非样式属性该如何实现，无法保证能够正常获取到插槽节点的元素，而且要怎么把属性传递进去？

竞品分析

| | webflow | plasmic |
| --- | --- | --- |
| 全局状态 | 响应式 | 响应式、自定义的业务状态 |
| 页面状态 | 不支持 | 支持 |
| 组件状态 | 支持交互状态，不支持业务状态变体样式 | 只支持交互状态，业务状态的变体样式需要使用自定义组件二次封装为变体属性来实现，并有只有基础组件支持，第三方和自定义组件都不支持 |
| 组合状态 | 不支持 | 自定义组件支持全局和组件组合状态，页面不支持组合状态，组件元素支持交互状态组合 |
...
- 非技术人员需要学习前端 CSS 布局知识，门槛过高
- 设计师习惯 figma 这类工具的操作模式

    - 从原子开始：对设计来说只有方块、文本和图片，没有标题、文本和按钮的区分，他们不会关心这些要使用不同类型的组件元素。例如：要在这个页面上放置一个按钮，设计师的第一反应是去画一个正方形在创建一个文本框，再把他们达成一个组。而不是去素材库里拖一个 Button 的组件，再调整它的各种参数。
    - 从自由到规则：设计师不会在最开始的时候去构思整个页面的适配规则，对于设计师来说，创作都是从一个固定的页面或者固定的大小开始的，在这个画布上去构思和绘制所有元素的展示和排布。而网页可视化编辑器是严格的自自顶向下，从外到里的配置（先考虑外层节点的布局方式，然后一层层往里配置）。
    - 直线理解：
    
        - 设计师不希望在使用一个工具的时候还要去做很多深层次的理解。举个例子，对于设计师来说，理解 CSS 的盒模型是一件有学习成本的事情，所以 Figma 在做 Auto Layout 时候简化了大部分关于 margin 的概念，基本上只保留了内部的 padding。这样让每个容器都只和自己可见的内部发生关系，如果要实现 margin 的做法是在外面再套一层容器。
        - 由于设计稿上所有的元素都是独立的，所以所有对象的参数也应该是互相独立的，基本不存在一个元素的某些属性 inherit 自它的父级的情况。
        - 在尺寸单位上，设计师的概念里只有一个标准就是 px，至于其他的 rem/em/vw/v
    
### 素材配置成本过高

问题描述

- 点选操作成本：每个素材在后台对应的是一条记录，而在可视化编辑器里我的素材可能是由多个组件元素组成的，图片在图片组件上配置，标题在文本组件上配置。我要在所有元素的图层里找到对应的元素，点击选择后才能进行配置，有时候配置事件和动画还可能选错元素，把它配置到子元素上了。
- 元素属性复杂且操作繁琐：一般可视化编辑器的属性面板分为四个部分：属性、样式、交互和动画，而原先在后台我的素材只要配置名称和资源，但是现在面板多了很多额外的信息需要管理，显示不够直观，并且增加了信息量和理解成本，随之我的操作成本也变高了。
- 重复类型的素材管理操作太过繁琐

    例如：我的页面有 100 个素材，我用管理系统我只要维护维护素材的名称和资源就行了，但是现在可视化编辑器里面增加大量的的点选操作成本。
    
    - 数据查看不够直观：在后台我可以在素材表格里直观的看到素材的信息，但是在可视化编辑器里我得点击某个元素才能看到隐藏在熟悉面板里的一些额外信息。
    - 新增数据操作成本高：上面提到了点选操作成本较高，元素属性面板复杂，这在重复批量的素材场景下更是如意。
    - 画布操作较为困难：素材对应的元素可能是多个，我在添加新素材的时候，容易把新素材插入到其他素材的内部节点里，类似的修改和删除也是存在同样的问题。

- ...

解决方案：

- widget 支持自定义属性，widge 子元素支持关联自定义属性，封装好使用时用户只要配置 widget 属性即可。
- 动态数据绑定：类似 plasmic 元素属性支持关联动态数据，并且支持 repeat 功能。
- 拆分出设计模式和配置模式

    - 配置模式：支持 CMS，类似于后台管理系统采用表格和表单的方式配置，并且支持自定义字段。
    - 设计模式：采用可视化的方式配置，结合 widget 和动态数据绑定，将元素的属性关联到 CMS 数据上。
